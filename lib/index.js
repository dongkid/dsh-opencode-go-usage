...
export function apply(ctx) {
  ...
  "gemini-2.5-flash": { in: 0.3, out: 2.5, cr: 0.03, cw: 0.0 },
    ...
    async function collectDshScan() {
      ...
      const snaps = await mapLimit(sessions, 6, (rec) => sq.readSession(rec.header.id).catch(() => null))
    ...
    function toOfficialData(parsed) {
      ...
      return { ok: true, vd: buildView(rows), rows, truncated: !!parsed.truncated, skippedPages: parsed.skippedPages || 0, records: rows.length, autoExtracted: !!parsed.autoExtracted, browser: parsed.browser || null }
    }
    ...
    let officialTruncRebuildAt = 0 // 截断缓存强制全量重建的 12h 节流
    let scanInflight = null // 后台会话扫描并发去重
    ...
    async function runOfficialHttp(envs) {
      ...
      // 页数上限 5000(25 万条):usage.list 历史超过 7500 条不再截断,抓取在"不足 50 条的页"自然结束
      const ABS_MAXP = 5000
      const MAXP = (cfg && Number.isFinite(cfg.maxPages) && cfg.maxPages > 0) ? Math.min(cfg.maxPages, ABS_MAXP) : ABS_MAXP
      const fetchPage = async (page) => {
        ...
        try {
          ...
          // 单页失败:短暂退避后重试一次;仍失败返回 null(由调用方跳过该页并计数,
          // 绝不因一页失败终止整次抓取——否则数据"抓不全")
          await new Promise((r) => setTimeout(r, 300))
          try { return await tryFetch() } catch (e2) { return null }
        }
      }
      try {
        let page = 0
        let stop = false
        let skippedPages = 0
        let emptyCount = 0 // 连续失败/空页计数:只有连续 5 页才判定数据尽头(防偶发抖动早停)
        ...
        while (page < MAXP && !stop) {
          ...
          for (let i = 0; i < batch.length; i++) {
            ...
            if (pgs === null || pgs.length === 0) {
              emptyCount++
              if (pgs === null) skippedPages++
              if (emptyCount >= 5) { stop = true; break }
              continue
            }
            emptyCount = 0
        ...
        if (lastValid) {
          ...
          if (!old || !Array.isArray(old.records)) {
            // 旧盘缺失/不可读:不回退到"保留旧缓存"——让调用方回退全量重建(自愈),
            // 错误码供 triggerIncremental 识别后降级 collectOfficial
            out.error = 'incremental disk cache unreadable; fallback to full fetch'
            return out
          }
          const seen = new Set()
          const combined = []
        ...
        // 4. 写盘缓存(带 truncated 标记:截断缓存由读取方触发全量重建)
        try {
          _ocgoMkdirSync(_ocgoJoin(_ocgoHomedir(), '.config'), { recursive: true })
          _ocgoWriteFileSync(diskPath, JSON.stringify({ at: Date.now(), records: out.records, truncated: out.truncated }), 'utf8')
        } catch (e) {}
        out.ok = true
      } catch (e) {
    ...
    // host 端读旧盘合并去重后写回,syncOfficialToCache 更新内存缓存。
    ...
    function triggerIncremental() {
      ...
      if (!disk || !disk.records.length) {
        // 磁盘缓存缺失:退化为全量重抓(collectOfficial 自带 15min 失败缓存退避,不会轰炸官网)
        return collectOfficial().catch(() => {})
      }
      const lastTs = disk.records.reduce((m, r) => (r.ts > m ? r.ts : m), '')
      if (!lastTs) return collectOfficial().catch(() => {})
      incrementalInflight = (async () => {
        try {
          const p = await runOfficial([['OCGO_LAST_TS', lastTs]])
          if (!p || !p.ok) {
            // 旧盘缺失/损坏(增量无合并基础)→ 回退全量重建,自愈而非永远看旧数据
            if (p && p.error && p.error.indexOf('fallback to full fetch') >= 0) {
              await collectOfficial().catch(() => {})
            }
            return
          }
    ...
    // 后台会话扫描(并发去重):完成后更新 lastScan,并用当前 official 行回填重算
    // 已缓存响应的 DSH 视图——客户端 fast-poll 自动拿到更新,不阻塞任何面板请求。
    function ensureScan() {
      if (scanInflight) return scanInflight
      scanInflight = (async () => {
        try {
          const rows = await collectDshScan()
          lastScan = { at: Date.now(), rows }
          if (cache && cache.data) {
            const off = cache.data.official
            const offRows = off && off.ok && off.rows ? off.rows : null
            const dshRows = backfillDsh(rows, offRows)
            const dv = buildView(dshRows)
            dv.matchedOfficial = dshRows.matchedOfficial || 0
            cache.data.dsh = dv
          }
        } catch (e) { /* 扫描失败:下次轮询再试 */ } finally {
          scanInflight = null
        }
      })()
      return scanInflight
    ...
    async function fetchAll() {
      ...
      inflight = (async () => {
        // DSH 会话扫描后台化:扫描可能数十秒,绝不阻塞面板响应(上游实测单次 64s 拖死轮询)。
        // lastScan 5 分钟内复用;过期/缺失时后台扫描,本次先用旧数据(无则空),完成后更新缓存。
        let dshRaw = lastScan ? lastScan.rows : []
        if (!lastScan || Date.now() - lastScan.at >= 5 * 60 * 1000) {
          ensureScan()
        }
        const quotaP = collectQuota().catch((e) => ({ error: 'quota: ' + String((e && e.message) || e) }))
        // 数据实时性:内存缓存(本会话已抓到的真实数据)直接展示,每次刷新增量抓
        // 最新页(1-3s);磁盘缓存**不再秒开旧数据**(用户要求:真实数据到位前显示
        // "加载中",不拿可能过期的缓存顶)——磁盘缓存只作增量基准(lastTs)与
        // 截断检测。失败缓存(ok:false)不算数据:collectOfficial 内部按 15min
        // 缓存退避去重,错误原样透传给面板展示。
        // 失败缓存(ok:false)也算 off:错误透传到面板 + 借 15min 缓存退避;
        // 但仅 ok 的缓存会被竞态兜底覆盖展示。
        let off = officialCache ? officialCache.data : null
        if (!off) {
          const disk = readOfficialDisk()
          if (disk && disk.records.length) {
            // 截断检测:新版盘带 truncated 字段;旧版(150 页上限时代)缓存无该字段
            // 且记录 ≥7500 视为截断 → 12h 节流内强制一次全量重建(自愈补齐历史)
            const legacyTrunc = !Object.prototype.hasOwnProperty.call(disk, 'truncated') && disk.records.length >= 7500
            if ((disk.truncated === true || legacyTrunc) && Date.now() - officialTruncRebuildAt >= 12 * 3600 * 1000) {
              officialTruncRebuildAt = Date.now()
              collectOfficial().catch(() => {})
            } else {
              // 有盘为增量基准:1-3s 拿到最新完整集,比全量快;完成后
              // syncOfficialToCache 自动更新,客户端 fast-poll 拿到数据
              triggerIncremental().catch(() => {})
            }
          } else {
            // 无磁盘缓存:官方全量抓取(10-15s,后台完成),期间显示加载中
            collectOfficial().catch(() => {})
          }
        } else if (off.ok) {
        ...
        const data = { ok: true, fetchedAt: Date.now(), quota: quota.error ? null : quota, quotaError: quota.error || null, dsh, dshLoading: !(lastScan && lastScan.rows), official: off || { ok: false, loading: true } }
        // 竞态兜底:后台抓取(增量/全量)可能早于本响应完成,其 sync 未命中
        // cache(当时 cache 还没赋值)——用最新 officialCache 覆盖 loading 占位,
        // 保证"数据到位后立即展示真实数据"而不是等到下一个轮询周期。
        if (officialCache && officialCache.data && officialCache.data.ok) data.official = officialCache.data
        // 官方视图"最近会话":usage.list 没有会话标题,按时间回填的 DSH 会话
        // 补上标题并按会话聚合(金额为官方回填值)。必须对最终 data.official
        // 操作:竞态覆盖可能刚换掉对象,只改 off 会被换掉导致 recent 空/旧;
        // id 用真实会话 id(曾用常量 's' 使 React key 全部相同,更新时列表错乱)。
        const finalOff = (data.official && data.official.ok && data.official.vd) ? data.official : null
        if (finalOff && dshRows.length) {
    ...
    async function launchDebugBrowser() {
      try {
        ...
        // 启动策略(2026-08-16 用户实测修复,上游 v1.6.22):
